                    ...
                    <tr>
                        ...
                        <td>
                                ...
                                <?php if($row['status'] == 3 && $review<=0): ?>
                                    ...
                                    <a class="dropdown-item submit_blog" href="javascript:void(0)" data-id="<?php echo $row['package_id'] ?>">Submit Blogs</a>
                                    <?php endif; ?>
...
function cancel_book(id) {
    ...
    $.ajax({
        ...
        method: "POST",
        data: { id: id, status: 2, calculate_cancellation_fee: 1 }, // Add the 'calculate_cancellation_fee' parameter
        dataType: "json",
        error: function (err) {
            console.log(err);
        ...
        success: function (resp) {
            ...
            if (typeof resp === "object" && resp.status === "success") {
                if (resp.cancellation_fee > 0) {
                    // Show the cancellation fee to the user
                    alert_toast(
                        "Book cancelled successfully. Cancellation refund: $" + resp.cancellation_fee.toFixed(2),
                        "success"
                    );
                } else {
...
$(function () {
    ...
    $(document).ready(function () {
        $('.submit_blog').on('click', function () {
            ...
            window.location.href = "blog_submission_form.php?package_id=" + packageId;
        });
    });
</script>
